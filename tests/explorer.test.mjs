...
test("each scenic has two chapters, three answerable tasks and three choice/two boolean questions",()=>{
  for(const scenic of SCENICS){assert.deepEqual(chaptersFor(scenic).map(c=>c.id),["story","observe"]);assert.deepEqual(tasksFor(scenic).map(t=>t.id),["look","together","question"]);const questions=questionsFor(scenic);assert.equal(questions.length,5);assert.equal(questions.filter(q=>q.type==="single").length,3);assert.equal(questions.filter(q=>q.type==="boolean").length,2);for(const q of questions){assert.ok(q.correct>=0&&q.correct<q.options.length)}assert.equal(questions[0].options[questions[0].correct],getCity(scenic.cityId).name)}
});
