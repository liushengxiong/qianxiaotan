...
export default function MobileShell({view="welcome",scenicId,cityId,mode="overview",setup=false}:Props){
  ...
  async function finishSetup(profile:MobileProfile){setData({...EMPTY_STATE,profile});setCity(profile.city);setSetupDone(true);setNotice("演示旅程已开启；刷新页面后会重新开始。");return true}
  const navigation=<><a href={"/explore?city="+city} aria-current={view!=="profile"&&view!=="assistant"?"page":undefined} className={view!=="profile"&&view!=="assistant"?"active":""}><IconMap stroke={1.8}/><span>探索地图</span></a><button onClick={()=>{setGuideScenic(scenic.id);setGuide(true)}} aria-expanded={guide} className={guide?"active":""}><XiaozhiArt/><span>小探陪你</span></button><a href="/my-journey" aria-current={view==="profile"?"page":undefined} className={view==="profile"?"active":""}><IconUser stroke={1.8}/><span>我的手账</span></a></>;
  const activities=[
  ...
  return <div className="mobile-surround"><main className={"mapp "+(showSetup?"is-onboarding":"")+" view-"+view}>
    <header className="m-top"><a href="/" className="m-logo"><i>黔</i><span>黔小探<small>让好奇心，走远一点</small></span></a>{ready&&!showSetup&&<nav className="desktop-navigation" aria-label="主导航">{navigation}</nav>}{ready&&!showSetup&&<div className="m-demo-state"><span>演示模式</span><small>刷新后重置</small></div>}{ready&&!showSetup&&<label className="m-city"><select aria-label="选择目的地" value={city} onChange={e=>{const next=e.target.value;if(view==="scenic")window.location.assign("/explore?city="+next);else{setCity(next);setGuideScenic("")}}}>{CITIES.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>}</header>
    ...
    <div className={"m-body "+(!showSetup&&["welcome","home","assistant"].includes(view)?"is-map-body":"")}>
      {!ready?<div className="m-state-screen" role="status"><XiaozhiArt pose="note"/><h1>正在翻开你的探索地图</h1><p>让好奇心，走远一点。</p></div>:showSetup?<MobileOnboarding initial={data.profile} busy={false} onComplete={finishSetup}/>:<>
        ...
        {view==="scenic"&&<section className="scenic-screen">
          ...
          <div className="scenic-screen-heading"><div className="m-mini-title"><span>{getCity(scenic.cityId).name} / {scenic.demo?"示例探索":"亲子探索"}</span><h1>{scenic.name}</h1></div><button className="scenic-guide-button" onClick={()=>setGuide(true)}><XiaozhiArt/><span>问问小探 ↗</span></button></div>
          ...
          {mode==="quiz"&&<MobileQuiz key={scenic.id} scenic={scenic} visit={visit} onComplete={async answers=>{updateVisit({answers,quiz_score:answers.filter((answer,index)=>answer===questionsFor(scenic)[index].correct).length*20});return true}}/>}
