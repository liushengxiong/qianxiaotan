...
import type {MobileVisit} from "../lib/mobile";
import {XiaozhiArt} from "./xiaozhi-art";

export function MobileQuiz({scenic,visit,onComplete}:{scenic:Scenic;visit?:MobileVisit;onComplete:(answers:number[])=>Promise<boolean>}){
  ...
  const score=answers.filter((answer,i)=>answer===questions[i].correct).length*20;
  async function next(){
