...
export function MobileProfileView({data,city}:{data:MobileState;city:string}){
  ...
  return <section className="m-profile">
    <header><XiaozhiArt pose="happy"/><div><span>每一步，都是新的发现</span><h1>{data.profile?.nickname||"小探险家"}的探索手账</h1><p>{getCity(city).name} · {data.profile?.role||"亲子"}同行 · 演示数据不保存</p></div><a href="/">重新规划 ↗</a></header>
    ...
    <div className="passport-content">
      ...
      {section==="scores"&&<section className="m-rank"><div className="m-section-title"><h2>我的趣味挑战成绩</h2><span>仅当前页面有效</span></div><p className="m-disclaimer">只呈现本次演示中的即时反馈；刷新后重置，不公开孩子资料。</p>{ranked.length?<ol>{ranked.map((v,i)=><li key={v.scenic_id}><i>{String(i+1).padStart(2,"0")}</i><a href={"/scenic/"+v.scenic_id+"?mode=quiz"}>{getScenic(v.scenic_id)?.name}<small>本页挑战 · 再试一次 →</small></a><b>{v.quiz_score}<small>分</small></b></li>)}</ol>:<a className="m-empty" href={"/scenic/"+first.id+"?mode=quiz"}>还没有成绩，开启第一场趣味挑战 →</a>}</section>}
      {section==="albums"&&<section className="m-album-profile"><div className="m-section-title"><h2>我们的电子相册</h2><span>在设备上生成与下载</span></div><div className="m-album-empty"><IconSlideshow stroke={1.8}/><h3>把照片变成一段会播放的回忆</h3><p>演示版不保存用户照片或相册。请选择一处景点，上传本地照片并下载生成的视频。</p><a href={"/scenic/"+first.id+"?mode=journal"} className="m-primary">制作第一本电子相册 →</a></div></section>}
    </div>
